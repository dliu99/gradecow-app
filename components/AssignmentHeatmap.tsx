...
function getIntensityClass(count: number): string {
  if (count === 0) return 'bg-stone-900'
...
export function AssignmentHeatmap({ assignments, courseNameMap, currentDate }: AssignmentHeatmapProps) {
  ...
  return (
    <View className="mb-8">
      <View className="bg-stone-800 rounded-2xl p-4">
          ...
          {calendarDays.map((day, index) => {
            ...
            return (
              <Pressable
                ...
                className={`rounded-lg items-center justify-center ${day.date ? getIntensityClass(count) : 'bg-transparent'} ${isToday ? 'border-2 border-stone-700' : ''}`}
              >
              </Pressable>
            )
          })}
