  ...
  for (const category of courseData.categories) {
    for (const assignment of category.assignments) {
      if (assignment.scoreModifiedDate && assignment.score !== null && !assignment.dropped) {
        const percentImpact = calculatePercentImpactFromRaw(
          assignment,
...
function CourseCardWithDetails({
  ...
  return <CourseCard course={course} lastUpdated={lastUpdated} recentImpact={recentImpact} />;
}
  ...
  return (
    <SafeAreaView className="flex-1 bg-neutral-900" edges={['top']} collapsable={false}>
      <ScrollView
        ...
        {isLoading ? (
          <Text className="mb-6 text-center text-stone-400">Loading...</Text>
        ) : courses && courses.length > 0 ? (
          <>
            <Text className="text-3xl font-bold text-white">Grades</Text>
            <View className="mt-4">
              {courses.map((course) => (
          ...
          <Text className="mt-8 text-center text-stone-400">No courses found</Text>
        )}
      </ScrollView>
    </SafeAreaView>
  );
